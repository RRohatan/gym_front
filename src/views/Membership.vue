    ...
    <div class="max-w-7xl mx-auto bg-white text-gray-800 shadow-xl rounded-2xl p-4 sm:p-8">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h1 class="text-xl sm:text-2xl font-bold ml-12 sm:ml-16 flex items-center gap-2">
          🏋️ Membresías <span class="text-sm font-normal text-gray-500 hidden sm:inline">| {{ tituloFiltro }}</span>
        </h1>
        <div class="flex gap-2 w-full md:w-auto">
          <router-link to="/Menu" class="btn btn-dark flex-1 md:flex-none justify-center">🏠 Inicio</router-link>
          <button @click="showModal = true" class="btn btn-success flex-1 md:flex-none justify-center text-sm">➕ Asignar</button>
        </div>
      </div>

      <div class="flex overflow-x-auto pb-2 gap-2 mb-4 scrollbar-hide">
        <button @click="filtrarMembresias('')" class="btn btn-sm whitespace-nowrap" :class="statusFilter === '' ? 'btn-primary' : 'btn-secondary'">Todos</button>
        <button @click="filtrarMembresias('active')" class="btn btn-sm whitespace-nowrap" :class="statusFilter === 'active' ? 'btn-primary' : 'btn-secondary'">✅ Activas</button>
        <button @click="filtrarMembresias('inactive_unpaid')" class="btn btn-sm whitespace-nowrap" :class="statusFilter === 'inactive_unpaid' ? 'btn-primary' : 'btn-secondary'">⏳ Por Pagar</button>
        <button @click="filtrarMembresias('expiring_soon')" class="btn btn-sm whitespace-nowrap" :class="statusFilter === 'expiring_soon' ? 'btn-primary' : 'btn-secondary'">⚠️ Vencen Pronto</button>
      </div>

      <div class="mb-4">
        <input v-model="busquedaMembresia" type="text" placeholder="🔍 Buscar miembro..." class="w-full px-4 py-2 border rounded-xl shadow-sm outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
        ...
        <table v-else class="w-full text-sm min-w-[800px]">
          ...
          <tbody class="divide-y divide-gray-100 bg-white">
            <tr v-if="membresiasFiltradas.length === 0">
              <td colspan="6" class="py-10 text-center text-gray-400">No se encontraron resultados.</td>
            </tr>
            <tr v-for="m in membresiasFiltradas" :key="m.id" class="hover:bg-gray-50 transition">
              ...
              <td class="py-3 px-4 text-center">
                <span class="px-2 py-1 rounded-full text-xs font-bold shadow-sm border"
                  :class="{
                    'bg-green-100 text-green-700 border-green-200': m.status === 'active',
                    'bg-red-100 text-red-700 border-red-200': m.status === 'expired',
                    'bg-yellow-100 text-yellow-700 border-yellow-200': m.status === 'inactive_unpaid',
                    'bg-gray-100 text-gray-600': m.status === 'cancelled'
                  }">
                  {{ traducirEstado(m.status) }}
                </span>
              </td>
              <td class="py-3 px-4 text-right font-mono font-bold">{{ formatCurrency(m.outstanding_balance) }}</td>
              <td class="py-3 px-4 text-center">
                <button class="btn btn-indigo btn-sm" @click="abrirEditarModal(m)">✏️ Editar</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="showEditModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
           ...
           <form @submit.prevent="guardarCambios" class="space-y-3">
             ...
             <div class="grid grid-cols-2 gap-3">
               <div>
                  ...
                  <input v-model="editarMembresia.start_date" type="date" class="w-full border p-2 rounded-lg" />
               </div>
               <div>
                  <label class="text-xs font-bold uppercase text-gray-500">Fin</label>
                  <input v-model="editarMembresia.end_date" type="date" class="w-full border p-2 rounded-lg" />
               </div>
             </div>
             ...
             <div class="flex justify-end gap-2 mt-4 pt-2 border-t">
               <button type="button" @click="cerrarEditarModal" class="btn btn-secondary">Cancelar</button>
       ...
       <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
         <div class="bg-white text-gray-800 w-full max-w-md p-6 rounded-xl shadow-2xl">
           <h2 class="font-bold text-lg mb-4 border-b pb-2">Asignar Nueva</h2>
           <form @submit.prevent="asignarMembresia" class="space-y-4">
             ...
             <div>
               ...
               <input v-model="busqueda" type="text" placeholder="Nombre..." class="w-full border p-2 rounded-lg" />
               <ul v-if="miembrosFiltrados.length" class="border rounded-lg mt-1 max-h-32 overflow-y-auto bg-gray-50 absolute w-64 z-10 shadow-lg">
                  <li v-for="m in miembrosFiltrados" :key="m.id" @click="seleccionarMiembro(m)" class="p-2 hover:bg-blue-100 cursor-pointer text-sm border-b">
                    {{ m.name }}
                  </li>
               </ul>
             </div>
...
const form = ref({ member_id: '', plan_id: '', end_date: '', status: 'active' })
const busqueda = ref('')

// Computed
const membresiasFiltradas = computed(() => {
  const term = busquedaMembresia.value.toLowerCase()
  return term ? membresias.value.filter(m => m.member?.name?.toLowerCase().includes(term)) : membresias.value
})

const miembrosFiltrados = computed(() => {
  const term = busqueda.value.toLowerCase()
  return term ? miembros.value.filter(m => m.name.toLowerCase().includes(term) || (m.email || '').toLowerCase().includes(term)) : []
})
...
const cargarMembresias = async () => {
  ...
  } catch (error) { console.error(error); membresias.value = [] } finally { loading.value = false }
}
...
// Acciones Modales
const seleccionarMiembro = (miembro) => { form.value.member_id = miembro.id; busqueda.value = miembro.name }
const cerrarModal = () => { showModal.value = false; busqueda.value = ''; form.value = { member_id: '', plan_id: '', status: 'active' } }

const asignarMembresia = async () => {
  try {
    let existing = null
    try { const res = await api.get(`/memberships/by-member/${form.value.member_id}`); existing = res.data } catch (e) {}

    if (existing) { alert(`Este cliente ya tiene membresía (${existing.status}).`); return }

    await api.post('/memberships', { member_id: form.value.member_id, plan_id: form.value.plan_id })
    cerrarModal()
    filtrarMembresias('inactive_unpaid')
    alert('Membresía asignada.')
  } catch (error) { console.error(error); alert('Error al asignar') }
}

const abrirEditarModal = (m) => {
  editarMembresia.value = { ...m, start_date: dayjs(m.start_date).format('YYYY-MM-DD'), end_date: dayjs(m.end_date).format('YYYY-MM-DD'), plan: { id: m.plan?.id } }
  showEditModal.value = true
}

const cerrarEditarModal = () => { showEditModal.value = false; editarMembresia.value = {} }
...
const traducirEstado = (s) => {
  const map = { active: 'Activa', expired: 'Vencida', inactive_unpaid: 'Por Pagar', cancelled: 'Cancelada' }
  return map[s] || s
}
