      ...
      <div v-else-if="member">
        ...
        <section>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <p>
              <span class="font-semibold">Fecha de nacimiento:</span> {{ member.birth_date || "—" }}
            </p>
          </div>
        </section>
        ...
        <section class="mt-6 text-gray-700">
          ...
          <p>
            ...
            {{
              member.estatura
                ? (member.estatura > 3 ? (member.estatura / 100).toFixed(2) : member.estatura) +
                  " m"
                : "—"
            }}
          </p>
          <p>
            <span class="font-semibold">Peso:</span> {{ member.peso ? member.peso + " kg" : "—" }}
          </p>
          <p>
            <span class="font-semibold">IMC:</span> {{ calcularIMC(member.peso, member.estatura) }}
          </p>
          <p>
        ...
        <section v-if="member.memberships?.length" class="mt-6">
          ...
          <div class="p-4 bg-blue-50 rounded-lg space-y-2 text-sm">
            ...
            <p>
              <span class="font-semibold">Precio:</span> {{ member.memberships[0].plan?.price }} COP
            </p>
          </div>
        </section>
      </div>
