<template>
  <div class="p-4 sm:p-6 min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
    ...
    <div v-if="modalMembresia" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white text-black w-full max-w-md p-6 rounded-lg shadow-lg">
        ...
        <form @submit.prevent="guardarMembresia">
          <label class="block mb-1 text-sm">Plan</label>
            <select
              v-model="nuevaMembresia.plan_id"
          ...
          <input v-model="nuevaMembresia.start_date" type="date" class="w-full mb-4 p-2 border rounded" required />
          <div class="flex justify-end gap-3">
            <button type="button" @click="cerrarModalMembresia" class="text-gray-600 px-4 py-2">Cancelar</button>
            <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded">Asignar</button>
          </div>
        </form>
      </div>
    </div>
